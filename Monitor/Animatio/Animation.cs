...
namespace Monitor
{
    public class Animation
    {
        ...
        public float Step()
        {
            float basicStep = Math.Abs(Value) / StepDivider;
            float resultStep = basicStep;

            if(Reverse == false)
            {
                if(Value <= p15 || Value >= p85)
                {
                    resultStep = basicStep / 3.15f;
                }
        ...
        private ControlMethod InvalildateControl;
        ...
        public void UpdateFrame()
        {
            ...
            InvalildateControl.Invoke();
        }
        public Animation() { }

        public Animation(string ID, ControlMethod InvalildateControl, float Value, float TargetValue)
        {
            this.ID = ID;

            this.InvalildateControl = InvalildateControl;
